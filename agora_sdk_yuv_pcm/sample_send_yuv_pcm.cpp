...
static void SampleSendAudioTask(
        ...
        agora::agora_refptr<agora::rtc::IAudioPcmDataSender> audioPcmDataSender,
        bool &exitFlag) {
    // Currently only 10 ms PCM frame is supported. So PCM frames are sent at 10ms interval
...
int main(int argc, char *argv[]) {
    ...
    optParser.add_long_opt("lowdelay", &options.low_delay, "enable the low delay");
    ...
    if (options.low_delay == true) {
        ...
        ret = s->setUInt("rtc.video.uplink_max_retry_times", options.video_resend);
        if (!ret) {
