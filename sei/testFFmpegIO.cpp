...
static int laifeng_read_cb(void *opaque, uint8_t *buf, int buf_size) {
    ...
    while (true) {
        ...
        */
        ...
        if (AVCPacketType == 0) {
            int configurationVersion = buffer[0];
            buffer++;
            unsigned char sps[3];
            sps[0] = buffer[0];
            buffer++;
            ...
            sps[2] = buffer[0];
            buffer++;
            int lengthSizeMinusOne = buffer[0] & 0x3;
            buffer++;
            int numSPS = buffer[0] & 0x1F;
            buffer++;
...
int testFFmpegIO(const char *path) {
    ...
    //laifeng_read_cb为回调函数
    AVIOContext *avioCtx = avio_alloc_context(IOBuffer, io_size,
                                              0, (void *) path,
                                              laifeng_read_cb, NULL, NULL);
    if (NULL == avioCtx) {
